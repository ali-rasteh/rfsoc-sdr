...
class RFSoC(General):
    ...
    def clock_init(self):
        ...
        xrfclk.set_ref_clks(
            lmk_freq=self.config.rfsoc_lmk_freq_mhz, lmx_freq=self.config.rfsoc_lmx_freq_mhz
        )
    ...
    def verify_clock_tree(self):
        if "ddr4" in self.config.project:
            ...
            if status != 1:
                raise Exception(
                    "The MTS ClockTree has failed to LOCK. Please verify board clocking configuration"
                )
        ...
        for tile_id in self.config.dac_tile_block_dic:
            for block_id in self.config.dac_tile_block_dic[tile_id]:
                ...
                if self.config.do_rfsoc_pll_settings:
                    dac_tile.DynamicPLLConfig(
                        self.config.DynamicPLLConfig[0],
    ...
    def load_data_to_tx_buffer(self, txtd):
        ...
        if "sounder_if" in self.config.project:
            txtd_dac_interleaved = np.zeros(np.prod(txtd_dac.shape) * 2, dtype="int16").reshape(
                -1, self.config.n_par_strms_tx // 2, 2
            )
            for i in range(self.config.n_tx_ant):
                ...
                if self.config.tx_mode == 1:
                    txtd_dac_interleaved[i :: self.config.n_tx_ant, :, 0] = np.int16(
                        txtd_dac_ant.real
                    )
                    txtd_dac_interleaved[i :: self.config.n_tx_ant, :, 1] = np.int16(
                        txtd_dac_ant.imag
                    )
            ...
            txtd_dac_interleaved = np.zeros(
                (
                    np.prod(txtd_dac.shape) * 2 // self.config.n_par_strms_tx,
                    self.config.n_par_strms_tx,
                ),
                dtype="int16",
            )
            for i in range(self.config.n_tx_ant):
                if self.config.tx_mode == 1:
                    txtd_dac_interleaved[i * 2 :: self.config.n_tx_ant * 2, :] = np.int16(
                        txtd_dac[i].real
                    )
                    txtd_dac_interleaved[i * 2 + 1 :: self.config.n_tx_ant * 2, :] = np.int16(
                        txtd_dac[i].imag
                    )
                elif self.config.tx_mode == 2:
                    txtd_dac_interleaved[i * 2 :: self.config.n_tx_ant * 2, :] = np.int16(
    ...
    def load_data_from_rx_buffer(self):
        rx_data = np.array(self.adc_rx_buffer).astype("int16") / (
            2 ** (self.config.adc_bits + 1) - 1
        )
        self.rxtd = []
        ...
        for i in range(self.config.n_rx_ant):
            if self.config.rx_mode == 1:
                rx_data_ant = (
                    rx_data[i * 2 :: self.config.n_rx_ant * 2, :]
                    + 1j * rx_data[i * 2 + 1 :: self.config.n_rx_ant * 2, :]
                )
            elif self.config.rx_mode == 2:
                rx_data_ant = (
                    rx_data[i * 2 + 1 :: self.config.n_rx_ant * 2, :]
