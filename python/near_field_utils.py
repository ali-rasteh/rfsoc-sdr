    ...
    def __post_init__(self):
        ...
        for k in range(self.nf_n_rx_loc_sep):
            for i in range(self.nf_n_ant_sep):
                ...
                t = (
                    self.nf_rx_loc_sep[k]
                    + self.nf_rx_ant_sep[i] * np.arange(self.n_rx_ant) * self.wl
                )
                # Position of the RX antennas
                self.nf_rx_ant_loc[:, m, :] = t[:, None] * self.nf_rx_sep_dir[None, :]

                t = self.ant_d_m[0] * np.arange(self.n_tx_ant)
                self.nf_tx_ant_loc[:, m, :] = (
                    self.nf_tx_loc + t[:, None] * self.nf_tx_sep_dir[None, :]
                )
...
class NearFieldSignalUtils(SignalUtilsRfsoc):
                ...
                if self.nf_sep_idx == 0:
                    if use_linear_track:
                        ...
                        if self.nf_loc_idx < len(self.nf_rx_loc):
                            distance = 1000 * (
                                self.nf_rx_loc[self.nf_loc_idx, 0]
    ...
    def est_nf_param(self, h, dly_est, peaks, npaths):
        ...
        self.nf_model.nf_channel_param_est(
            ...
            lr_init=lr_init,
            H_gt=H_gt,
            tx_ant_vec=tx_ant_vec,
            rx_ant_vec=rx_ant_vec,
            trx_unit_vec=trx_unit_vec,
