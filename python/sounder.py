...
class SounderConfig(GeneralConfig):
    ...
    ant_d_m = [0.02]                # Antenna axis spacing in meters, a list of spacing for each axis, for example [0.02, 0.02] for a 2D array with 2 cm spacing in both axes
    ...
    npath_max = [20,5]                      # 1st number is the maximum number to extract at the 1st round, 2nd number is the maximum number to extract at the 2nd round
    ...
    steer_rad = [np.deg2rad(0.0), np.deg2rad(0.0)]      # Desired steering angles in radians [azimuth, elevation]
    ...
    def __post_init__(self):
        ...
        self.config_save_path = os.path.join(self.config_dir, 'config.json')        # Configuration save path
        ...
        self.ant_d = [d / self.wl for d in self.ant_d_m]   # Antenna axis spacing in wavelengths (lambda)
