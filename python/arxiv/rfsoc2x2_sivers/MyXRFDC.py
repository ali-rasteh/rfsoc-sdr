...
def _unpack_value(value):
    ...
    # Struct -> Dict
        ...
        return PropertyDict(
            {
                k: _unpack_value(getattr(value, k))
                if ktype.type.kind != "primitive"
    ...
    # Array -> List
        ...
        return [
            _unpack_value(value[i]) if ctype.item.kind != "primitive" else value[i]
...
def _create_c_property(
    name, typename, readonly, writeonly=False, implicit_read=False, implicit_write=False
):
    def _get(self):
        value = _ffi.new(f"{typename}*")
        c_func = self._call_function if not implicit_read else self._call_function_implicit
        c_func(f"Get{name}", value)
        value = _unpack_value(value)
        if isinstance(value, PropertyDict):
            value.set_callback(lambda value: c_func(f"Set{name}", _pack_value(typename, value)))
        return value
...
class RFdc(pynq.DefaultIP):
    ...
    def __init__(self, description):
        ...
        if "parameters" in description:
            ...
            warnings.warn(
                "Please use an hwh file with the RFSoC driver the driver cannot be loaded"
            )
    ...
    def runMTS(self):
        ...
        while True:
            ...
            statusMTSDAC = _lib.XRFdc_MultiConverter_Sync(
                self._instance, _lib.XRFDC_DAC_TILE, self._DAC_Sync_Config
            )
            print(
                "Status:"
                + str(statusMTSDAC)
                + ", DAC 0: Latency:"
                + str(self._DAC_Sync_Config.Latency[0])
                + " Offset:"
                + str(self._DAC_Sync_Config.Offset[0])
                + ", DAC 2: Latency:"
                + str(self._DAC_Sync_Config.Latency[1])
                + " Offset:"
            ...
            if (
                statusMTSDAC == 0
                and self._DAC_Sync_Config.Offset[0] == self._DAC_Sync_Config.Offset[1]
            ):
        ...
        while True:
            ...
            statusMTSADC = _lib.XRFdc_MultiConverter_Sync(
                self._instance, _lib.XRFDC_ADC_TILE, self._ADC_Sync_Config
            )
            print(
                "Status:"
                + str(statusMTSADC)
                + ", ADC 0: Latency:"
                + str(self._ADC_Sync_Config.Latency[0])
                + " Offset:"
                + str(self._ADC_Sync_Config.Offset[0])
                + ", ADC 2: Latency:"
                + str(self._ADC_Sync_Config.Latency[2])
                + " Offset:"
            ...
            if (
                statusMTSADC == 0
                and self._ADC_Sync_Config.Offset[0] == self._ADC_Sync_Config.Offset[2]
            ):
        ...
        for i in range(0, 4):
            if (1 << i) & self._DAC_Sync_Config.Tiles:
                print(
                    "DAC{}: Latency(T1) = {}, Adjusted Delay Offset() = {}, Marker Delay = {}".format(
                        i,
                ...
                print(
                    "    DAC{}: PLL DTC Code ={} ".format(
                        i, self._DAC_Sync_Config.DTC_Set_PLL.DTC_Code[i]
                    )
                )
                print(
                    "    DAC{}: PLL Num Windows ={} ".format(
                        i, self._DAC_Sync_Config.DTC_Set_PLL.Num_Windows[i]
                    )
                )
                print(
                    "    DAC{}: PLL Max Gap ={} ".format(
                        i, self._DAC_Sync_Config.DTC_Set_PLL.Max_Gap[i]
                    )
                )
                print(
                    "    DAC{}: PLL Min Gap ={} ".format(
                        i, self._DAC_Sync_Config.DTC_Set_PLL.Min_Gap[i]
                    )
                )
                print(
                    "    DAC{}: PLL Max Overlap ={} ".format(
                        i, self._DAC_Sync_Config.DTC_Set_PLL.Max_Overlap[i]
                    )
                )
                print("=== MTS DAC Tile{} T1 Report ===".format(i))
                print(
                    "    DAC{}: T1 DTC Code ={} ".format(
                        i, self._DAC_Sync_Config.DTC_Set_T1.DTC_Code[i]
                    )
                )
                print(
                    "    DAC{}: T1 Num Windows ={} ".format(
                        i, self._DAC_Sync_Config.DTC_Set_T1.Num_Windows[i]
                    )
                )
                print(
                    "    DAC{}: T1 Max Gap ={} ".format(
                        i, self._DAC_Sync_Config.DTC_Set_T1.Max_Gap[i]
                    )
                )
                print(
                    "    DAC{}: T1 Min Gap ={} ".format(
                        i, self._DAC_Sync_Config.DTC_Set_T1.Min_Gap[i]
                    )
                )
                print(
                    "    DAC{}: T1 Max Overlap ={}".format(
                        i, self._DAC_Sync_Config.DTC_Set_T1.Max_Overlap[i]
                    )
                )
        ...
        for i in range(0, 4):
            if (1 << i) & self._ADC_Sync_Config.Tiles:
                ...
                print("=== MTS DAC Tile{} PLL Report ===".format(i))
                print(
                    "    ADC{}: PLL DTC Code ={} ".format(
                        i, self._ADC_Sync_Config.DTC_Set_PLL.DTC_Code[i]
                    )
                )
                print(
                    "    ADC{}: PLL Num Windows ={} ".format(
                        i, self._ADC_Sync_Config.DTC_Set_PLL.Num_Windows[i]
                    )
                )
                print(
                    "    ADC{}: PLL Max Gap ={} ".format(
                        i, self._ADC_Sync_Config.DTC_Set_PLL.Max_Gap[i]
                    )
                )
                print(
                    "    ADC{}: PLL Min Gap ={} ".format(
                        i, self._ADC_Sync_Config.DTC_Set_PLL.Min_Gap[i]
                    )
                )
                print(
                    "    ADC{}: PLL Max Overlap ={} ".format(
                        i, self._ADC_Sync_Config.DTC_Set_PLL.Max_Overlap[i]
                    )
                )
                print("=== MTS ADC Tile{} T1 Report ===".format(i))
                print(
                    "    ADC{}: T1 DTC Code ={} ".format(
                        i, self._ADC_Sync_Config.DTC_Set_T1.DTC_Code[i]
                    )
                )
                print(
                    "    ADC{}: T1 Num Windows ={} ".format(
                        i, self._ADC_Sync_Config.DTC_Set_T1.Num_Windows[i]
                    )
                )
                print(
                    "    ADC{}: T1 Max Gap ={} ".format(
                        i, self._ADC_Sync_Config.DTC_Set_T1.Max_Gap[i]
                    )
                )
                print(
                    "    ADC{}: T1 Min Gap ={} ".format(
                        i, self._ADC_Sync_Config.DTC_Set_T1.Min_Gap[i]
                    )
                )
                print(
                    "    ADC{}: T1 Max Overlap ={}".format(
                        i, self._ADC_Sync_Config.DTC_Set_T1.Max_Overlap[i]
                    )
                )
    ...
    def sysrefDisable(self):
        status3 = _lib.XRFdc_MTS_Sysref_Config(
            self._instance, self._DAC_Sync_Config, self._ADC_Sync_Config, 0
        )
    ...
    def sysrefEnable(self):
        status3 = _lib.XRFdc_MTS_Sysref_Config(
            self._instance, self._DAC_Sync_Config, self._ADC_Sync_Config, 1
        )
        if status3 == _lib.XRFDC_MTS_OK:
...
for name, typename, readonly, writeonly, implicit_read, implicit_write in _adc_props:
    setattr(
        RFdcAdcBlock,
        name,
        _create_c_property(name, typename, readonly, writeonly, implicit_read, implicit_write),
    )
